...
export const insertEmployee_infoCon = async (req, res) => {
  try {
    ...
    const [result] = await pool.query(
      "INSERT INTO employee_info (name, position, department, email) VALUES (?, ?, ?, ?)",
...
export const updateEmployee_infoCon = async (req, res) => {
  try {
    ...
    const { name, position, department, email } = req.body;
    await pool.query(
      "UPDATE employee_info SET name = ?, position = ?, department = ?, email = ? WHERE employee_id = ?",
      [name, position, department, email, employee_id]
    );
...
export const insertpayrollCon = async (req, res) => {
  try {
    ...
    const [result] = await pool.query(
      "INSERT INTO payroll (employee_id, salary, bonus, deductions) VALUES (?, ?, ?, ?)",
...
export const updatepayrollCon = async (req, res) => {
  try {
    ...
    const { employee_id, salary, bonus, deductions } = req.body;
    await pool.query(
      "UPDATE payroll SET employee_id = ?, salary = ?, bonus = ?, deductions = ? WHERE id = ?",
      [employee_id, salary, bonus, deductions, id]
    );
