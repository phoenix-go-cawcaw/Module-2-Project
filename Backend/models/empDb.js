...
const insertEmployee_InfoDb = async (name, position, department, salary, employment_history, contact) => {
    let [data] = await pool.query('INSERT INTO `employee_info` (`name`, `position`, `department`, `salary`, `employment_history`, `contact`) VALUES (?, ?, ?, ?, ?, ?);', [name, position, department, salary, employment_history, contact])
    return data
}

const insertpayrollDb = async (employee_id, hours_worked, leave_deductions, final_salary,) => {
    let [data] = await pool.query('INSERT INTO `payroll` (`employee_id`, `hours_worked` , `leave_deductions` , `final_salary`) VALUES (?, ?, ?, ?);', [employee_id, hours_worked, leave_deductions, final_salary])
    return data
}

const insertattendanceDb = async (employee_id, attendance_date ,status) => {
    let [data] = await pool.query('INSERT INTO `attendance` (`employee_id`,`attendance_date`, `status`) VALUES (?, ?, ?)', [employee_id, attendance_date, status])
    return data
}

const insertleave_requestDb = async (employee_id, leave_request ,leave_type, status) => {
    let [data] = await pool.query('INSERT INTO `leave_requests` (`employee_id`,`leave_request`,`leave_type`, `status`) VALUES (?, ?, ?, ?)', [employee_id, leave_request, leave_type, status])
    return data
}
...
const updateleave_requestsDb = async (leave_id, updates, employee_id) => {
  const fields = []
  const values = []
